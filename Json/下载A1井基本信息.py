...
if __name__ == "__main__":
    ...
    WellCardDataSavePath = r"F:\冀东油田数据库应用系统\冀东油田-数据库-提取地址\A1-2.0数据\单井卡片"
    ...
    for row in AllWell_Json["result"]:
        ...
        pass
    file = open((WellCardDataSavePath + "\\单井卡片.json"), "w", 1)
    # 6、写入文件
    str1 = str(result).replace("\\r\\n", "").replace(" 00:00:00", "")
    file.write(str1)
    # 7、关闭文件
    file.close()
    print("下载完成！")
    ...
    with open((WellCardDataSavePath + "\\单井卡片.csv"), 'w', newline='', encoding='gb18030') as f:
        writer = csv.writer(f)
